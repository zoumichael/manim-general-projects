...
#test 1
tls = tls + [[r"The core concept in calculus:  limit", TV, TIT]]
count = count + 1
...
count = count + 1
tls = tls + [[r"1. A number $m$ is called as a {\bf lower bound} of $S$ if $m \le x$ for ANY $x\in S$.", TV, DEF]]
count = count + 1
tls = tls + [[r"$S$ is called {\bf lower bounded} if such a lower bound exists;", TV, DEF]]
count = count + 1
tls = tls + [[r"2. A value $M$ is called as a {\bf upper bound} of $S$ if $M \ge x$  for ANY $x\in S$.", TV, DEF]]
count = count + 1
tls = tls + [[r"$S$ is called {\bf upper bounded} if such a upper bound exists;", TV, DEF]]
count = count + 1
tls = tls + [[r"3. $S$ is called {\bf bounded} if it is both lower bounded and upper bounded.", TV, DEF]]
group_ends = group_ends + [count+1]

count = count + 1
tls = tls + [[r"Remark:", TV,REM]]
count = count + 1
tls = tls + [[r"1. Above lower bound $m$ or upper bound $M$  is not required to be in $S$. "
              r"For example, $S=(1,2)$ is bounded with $0$ and $3$ as a lower bound and upper bound respectively.  notice that $0$ and $3$ are not in $S$.", TV,REM]]
count = count + 1
tls = tls + [[r"2. lower bound and upper bound are not unique. $1$ and $2$ are another lower bound and upper bound for above $S$.", TV, REM]]
group_ends = group_ends + [count+1]
...
count = count + 1
tls = tls + [[r"Statement.", TV, STA]]
count = count + 1
tls = tls + [[r"If $m$ is not a lower bound of a set $S$,  there exist $x\in S$ such that $x<m$.", TV, STA]]
count = count + 1
...
tls = tls + [[r"Def 1.1.1.", TV, DEF]]
count = count + 1
tls = tls + [[r"Let $S$ be a general set of numbers, A number $m$ is called the greatest lower bound of $S$ if ", TV, DEF]]
count = count + 1
tls = tls + [[r"  a) $ m \le x $  for all $x$ in S;", TV, DEF]]
count = count + 1
tls = tls + [[r"  b) For any positive number $\epsilon$, $m + \epsilon$ is no longer a lower bound, i.e.  there exist $x_0\in S$ such that $x_0 < m + \epsilon$.", TV, DEF]]
count = count + 1
tls = tls + [[r"    The greatest lower bound is denoted by $\inf S$, called the infimum of $S$. We write $\inf S=-\infty$ if it does not exist.", TV, DEF]]
count = count + 1
tls = tls + [[r"A number $M$ is called the least upper bound of $S$ if ", TV, DEF]]
count = count + 1
tls = tls + [[r"  a) $ m \le x $  for all $x$ in S;", TV, DEF]]
count = count + 1
tls = tls + [[r"  b) For any positive number $\epsilon$, $M-\epsilon$ is no longer a upper bound, i.e.  there exist $y_0\in S$ such that $y_0 > M - \epsilon$", TV, DEF]]
count = count + 1
tls = tls + [[r"    The least upper bound is denoted by $\sup S$, called the supremum of $S$. We write $\sup S=\infty$ if it does not exist.", TV, DEF]]
group_ends = group_ends + [count+1]
...
count = count + 1
tls = tls + [[r"$S=(1,2)$. It is clear that $\inf S = 1$ and $\sup S = 2$", TV, EXA]]
count = count + 1
tls = tls + [[r"$S= (1,2,3,...)$,  the set of all natural numbers.  $S$ is lower bounded with $\inf S=1$ while $\sup S = \infty$ since it is not upper bounded", TV, EXA]]
group_ends = group_ends + [count+1]
...
tls = tls + [[r" A sequence $A$ in Eq (1.1) is called increasing (decreasing) if any item in the sequence is always not larger than its following item, i.e. \\"
      r"     $a_n\le a_{n+1}$ ($a_n\ge a_{n+1}$) for any $n\ge 1$.  \\"
              r"$A$ is called monotonic if it is either increasing or decreasing.", TV, DEF]]
count = count + 1
...
count = count + 1
tls = tls + [[r"$\lim_{n\to \infty}a_n := \sup A;$", TV, EQU]]
count = count + 1
tls = tls + [[r"The limit of a bounded decreasing sequence $A$ is defined to be $\inf A$, denoted by", TV, DEF]]
count = count + 1
tls = tls + [[r"$\lim_{n\to \infty}a_n := \inf A$;", TV, EQU]]
group_ends = group_ends + [count+1]
...
count = count + 1
tls = tls + [[r"$|a_n-l|<\epsilon, \quad \quad  \forall n\ge N$.", TV, DEF]]
count = count + 1
tls = tls + [[r"In this case, we write", TV, DEF]]
count = count + 1
tls = tls + [[r"$\lim_{n\to \infty} a_n = l \qquad or \qquad a_n\to l$,", TV, DEF]]
count = count + 1
...
count = count + 1
tls = tls + [[r"It is increasing with $\sup A =1$.", TV, PRO]]
count = count + 1
tls = tls + [[r"$\lim_{n\to \infty} \frac{1}{n^{2+(-1)^n}} = 0$.", TV, EXA]]
count = count + 1
tls = tls + [[r"It is neither increasing nor decreasing. Let us prove it by definition. ", TV, PRO]]
count = count + 1
tls = tls + [[r"  For any $\epsilon>0$, we need find required $a_N$.", TV, PRO]]
count = count + 1
tls = tls + [[r"  Since $0<a_n\le \frac1n$, "
      r"to make $|a_n-0|<\epsilon$, we only need $\frac1n<\epsilon$.", TV, PRO]]
count = count + 1
tls = tls + [[r"  If we choose $N$ be any integer that is larger than $\frac1\epsilon$, then"
      r"$$|a_n-l|=a_n\le \frac1n<\frac1N<\epsilon, \quad  \forall n\ge N.$$ ", TV, PRO]]
count = count + 1
tls = tls + [[r"  Since $0<a_n\le \frac1n$,  "
      r"to make $|a_n-0|<\epsilon$, we only need $\frac1n<\epsilon$. If we choose $N$ be any integer that is larger than $\frac1\epsilon$, then"
      r"$$|a_n-l|=a_n\le \frac1n<\frac1N<\epsilon, \quad  \forall n\ge N.$$ ", TV, PRO]]
count = count + 1
tls = tls + [[r"$\lim_{n\to \infty} (1+(-1)^n)$ does not exist.", TV, EXA]]
...
tls = tls + [[r"Intuitively, the sequence does not approach to a given value since $a_n$ is alternating between $0$ and $2$.  "
      ...
      r"there is no $N$ that meets Inequlity (1.7). ", TV, PRO]]
group_ends = group_ends + [count+1]
...
count = count + 1
tls = tls + [[r"Let $S_n = \sum_{1\le i\le n}a_i$ be the sum of the first $n$ terms in $A$.", TV, DEF]]
count = count + 1
tls = tls + [[r"If $S_n$ converges to certain value $S$, then the sequence is called summable and $S$ is defined as the sum of the sequence $A$. We write", TV, DEF]]
count = count + 1
...
count = count + 1
tls = tls + [[r"He shall never catch the tortoise!", TV, TOP]]
count = count + 1
tls = tls + [[r"Each time he reaches to the previous position of the tortoise, ", TV, TOP]]
count = count + 1
tls = tls + [[r"the tortoise has moved forward to a new position ahead of him as demonstrated below.", TV, TOP]]
count = count + 1
tls = tls + [[r"We see that Achilles and the tortoise start at two positions $A_0$ and $T_0$ respectively and ",  TV, TOP]]

count = count + 1
tls = tls + [[r"the tortoise moves ahead to $T_{i}$ when Achilles catches up to $A_i=T_{i-1}$ for each $i=1,2,3,...$.", TV, TOP]]
...
count = count + 1
tls = tls + [[r"$\cos(x) = 1- \frac{1}{2!}x^2  + \frac{1}{4!} x^4 - \frac{1}{6!} x^6  \dots , \quad x\in R$", TV, EQU]]
group_ends = group_ends + [count+1]
count = count + 1
...
% \nonumber % Remove numbering (before each equation)
  |\frac{\pi}4 -S_n| &=& |\frac1{2n+1}-\frac1{2n+3}+\frac1{2n+5} - \frac1{2n+7}+ \dots| \\
   &=& |(\frac1{2n+1}-\frac1{2n+3})+(\frac1{2n+5}- \frac1{2n+7}) -\dots | \\
   &=& (\frac1{2n+1}-\frac1{2n+3})+(\frac1{2n+5}- \frac1{2n+7})- \dots   \\
   &=&  \frac1{2n+1}-(\frac1{2n+3}-\frac1{2n+5}) - (\frac1{2n+7}-\frac1{2n+9})  - \dots  \\
   &<& \frac1{2n+1} 
\end{eqnarray*}
}
'''
...
Mathematics is the queen of science and arithmetrivs is the queen of mathematics

you can vidualize dervative
what function you're looking at and how you think about tiny nudges to its output
we care about dervatives because they hlpe us solve problmes 
in out little exploration here, we arldady have slight glimpse of one way that they're used 
they are the key to solve integral questions, problems that reqires to find area under a curve
once you gain enough familiarity with computing dervatives, you will be able to look at a situation like this where you do not know what a functionis but you do know that its dervative sahoud be x^2
and from that reverse engineer what the function must be
and this back and forth bewteen integral and dervative 
where the dervative of a function for the area under a graph gives you back the function defining the graph itself is called FTM of calculus
it ties together the two big ideas of inegrals and dervatives 
it shows how, in some sense, each one is an inverse of the other
all of these is only high-level view
just a peek at some of the core ideas that emerge in calculus, and what follows in the series are the details for dervatives and integrals and more
At all points,  i want you to feel that you could've invented calculus yourself. 
that if you drew the right pictures and played with each idea in just the right way,  these fomujlas and rules and constructs that are presented could have
just as eaisly popped out naturally from your own explorations, and before you go  
