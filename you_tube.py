...
class Mnemonics(Scene):
    def construct(self):
        ...
        # TRIG POST
        sin_vals_p = VGroup(*[
            Tex(t)[0] for t in ["0", "\\frac{1}{2}", "\\frac{\\sqrt{2}}{2}", "\\frac{\\sqrt{3}}{2}", "1"]
        ])
        cos_vals_p = VGroup(*[
            Tex(t)[0] for t in ["0", "\\frac{1}{2}", "\\frac{\\sqrt{2}}{2}", "\\frac{\\sqrt{3}}{2}", "1"][::-1]
        ])
        tan_vals_p = VGroup(*[
            Tex(t)[0] for t in ["0", "\\frac{\\sqrt{3}}{3}", "1", "\\sqrt{3}", "\\infty"]
        ])
...
class InscribedAngle(MovingCameraScene):
    ...
    def proof_1(self, case):
        ...
        self.wait(2)
        # formulas develop
        t1 = Tex("\\psi", "+", "2", "\\theta", "=", "180^\\circ",
                        tex_to_color_map={
                            "\\psi": psi.get_color(), "\\theta": theta.get_color(),
                        },
                        )
        t2 = Tex("\\psi", "+", "\\varphi", "=", "180^\\circ",
                        tex_to_color_map={
                            "\\psi": psi.get_color(), "\\varphi": varphi.get_color(),
                        },
                        )
        t3 = Tex("\\psi", "+", "2", "\\theta", "=", "\\psi", "+", "\\varphi",
                        tex_to_color_map={
                            "\\psi": psi.get_color(), "\\varphi": varphi.get_color(),
                            "\\theta": theta.get_color()
                        },
                        )
        t4 = Tex("2", "\\theta", "=", "\\varphi",
                        tex_to_color_map={
                            "\\psi": psi.get_color(), "\\varphi": varphi.get_color(),
                            "\\theta": theta.get_color()
                        },
    ...
    def proof_3(self, case):
        ...
        f = VGroup(*[
            Tex(*formula, **tex_formulas_kwargs)
